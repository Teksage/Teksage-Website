...
import {
  fetchEventDivisionalCharts,
  fetchEventHoroscopeDasa,
  fetchEventHoroscopeAshtaVarga,
  fetchEventHoroscopeSpecialLagna,
  fetchEventHoroscopeShadbala,
  fetchEventHoroscopeBhavaPosition,
  fetchEventHoroscopePlanetaryPosition,
} from "@/lib/services/event-full-horoscope";
import type {
  DivisionalChart,
  DasaPayload,
  AshtaVargaPayload,
...
/** Full Horoscope sections for a consultation customer (astrologer). */
export function useEventFullHoroscope(eventId: string | null): FullHoroscopeState & {
  enabled: boolean;
} {
  const enabled = Boolean(eventId?.trim());
  ...
  const load = useCallback(() => {
    ...
    Promise.allSettled([
      fetchEventDivisionalCharts(id),
      fetchEventHoroscopeDasa(id),
      fetchEventHoroscopeAshtaVarga(id),
      fetchEventHoroscopeSpecialLagna(id),
      fetchEventHoroscopeShadbala(id),
      fetchEventHoroscopeBhavaPosition(id),
      fetchEventHoroscopePlanetaryPosition(id),
