import { HOME_DASHBOARD_UI } from "@/lib/constants";
import { cn } from "@/lib/utils";
import type { EventPlannerCtaArrowProps } from "@/types";

/** Orange circle arrow for Event Planner (Muhurtha) banner CTA — matches card accent. */
export function EventPlannerCtaArrow({ className }: EventPlannerCtaArrowProps) {
  return (
    <span className={cn(HOME_DASHBOARD_UI.eventPlannerBannerCtaArrow, className)} aria-hidden>
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect
          width="14"
          height="14"
          rx="7"
          className="fill-[var(--color-muhurtha-card-accent)]"
        />
        <path
          d="M3 6.5C2.72386 6.5 2.5 6.72386 2.5 7C2.5 7.27614 2.72386 7.5 3 7.5V6.5ZM11.3536 7.35355C11.5488 7.15829 11.5488 6.84171 11.3536 6.64645L8.17157 3.46447C7.97631 3.2692 7.65973 3.2692 7.46447 3.46447C7.2692 3.65973 7.2692 3.97631 7.46447 4.17157L10.2929 7L7.46447 9.82843C7.2692 10.0237 7.2692 10.3403 7.46447 10.5355C7.65973 10.7308 7.97631 10.7308 8.17157 10.5355L11.3536 7.35355ZM3 7.5H11V6.5H3V7.5Z"
          className="fill-white"
        />
      </svg>
    </span>
  );
}
